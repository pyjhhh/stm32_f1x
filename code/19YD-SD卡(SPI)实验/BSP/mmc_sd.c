...
u8  SD_Type=0;//SD卡的类型 
 ...
 /**
  * @file   SD_SPI_ReadWriteByte
  * @brief  SPI读写一个字节（发送完成后返回本次通讯读取的数据）
  * @param  data:要写入的数据
  * @retval 读到的数据
  */
u8 SD_SPI_ReadWriteByte(u8 data)
{
 ...
 /**
  * @file   SD_SPI_SpeedLow
  * @brief  SPI设置速度为低速(SD卡初始化的时候,需要低速)
  * @param  无
  * @retval 无
  */
void SD_SPI_SpeedLow(void)
{
 ...
 /**
  * @file   SD_SPI_SpeedHigh
  * @brief  SPI设置速度为高速(SD卡正常工作的时候,可以高速了)
  * @param  无
  * @retval 无
  */
...
u32 SD_GetCapacity(void)
{
 ...
 u16 csize;
    ...
    if((csd[0]&0xC0)==0x40)
    {    
 			csize=csd[9]+((u16)csd[8]<<8)+1;     //扇区数
			Capacity=(u32)csize*512;             //容量M
    }
    else
    {
        /**********************************************/
